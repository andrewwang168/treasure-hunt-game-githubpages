...
export default function App({ onShowAuth }: { onShowAuth?: () => void }) {
  ...
  useEffect(() => {
    if (gameEnded && currentUser) {
      const result = score > 0 ? 'win' : score === 0 ? 'tie' : 'loss';
      saveScore(score, result).catch(console.error);
    }
  }, [gameEnded]);
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-amber-50 to-amber-100 flex flex-col items-center p-8 relative">
      <div className="absolute top-4 right-4 flex items-center gap-3">
        {currentUser ? (
          <>
            <span className="text-amber-700 text-sm">{currentUser.email}</span>
            <Button size="sm" onClick={() => { signOut(); toast.success('Signed out'); }} className="bg-amber-600 hover:bg-amber-700 text-white">Sign Out</Button>
          </>
        ) : (
          <Button size="sm" onClick={onShowAuth} className="bg-amber-600 hover:bg-amber-700 text-white">Sign In / Sign Up</Button>
        )}
      </div>
      ...
      <div className="text-center mt-12 mb-6">
        ...
        {!currentUser && (
          <p className="text-amber-600 text-sm mt-1">Playing as guest - scores won't be saved</p>
        )}
      </div>

      <div className="mb-4 relative z-10">
        <div translate="no" className="text-2xl text-center p-4 bg-amber-200/80 backdrop-blur-sm rounded-lg shadow-lg border-2 border-amber-400">
          <span className="text-amber-900">Current Score: </span>
          <span className={`${score >= 0 ? 'text-green-600' : 'text-red-600'}`}>
            ${score}
          </span>
          {gameEnded && (
            <span className={`ml-3 text-xl font-bold ${score > 0 ? 'text-green-600' : score === 0 ? 'text-yellow-600' : 'text-red-600'}`}>
      ...
      <div className="relative pt-8">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
              {boxes.map((box) => (
                <motion.div
                  ...
                  style={!box.isOpen ? { cursor: `url(${keyIcon}), pointer` } : { cursor: 'default' }}
        ...
        {gameEnded && (
          ...
          >
            <div className="mb-4 p-6 bg-amber-200/90 backdrop-blur-sm rounded-xl shadow-lg border-2 border-amber-400 text-center">
              <h2 className="text-2xl mb-2 text-amber-900">Game Over!</h2>
              <p translate="no" className="text-lg text-amber-800">
